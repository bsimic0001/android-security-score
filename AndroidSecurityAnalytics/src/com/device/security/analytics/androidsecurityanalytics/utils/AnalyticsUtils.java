...
public class AnalyticsUtils {
	...
	/*
	 * 
	 * READ_PHONE_STATE 1385 94.93% INTERNET 1380 94.59% READ_EXTERNAL_STORAGE
	 * 1299 89.03% WRITE_EXTERNAL_STORAGE 1299 89.03% ACCESS_NETWORK_STATE 1137
	 * 77.93% ACCESS_WIFI_STATE 817 62.46% READ_SMS 797 54.63%
	 * RECEIVE_BOOT_COMPLETED 708 48.53% WRITE_SMS 670 45.92% SEND_SMS 655
	 * 44.89% RECEIVE_SMS 520 35.64% VIBRATE 496 34.00% ACCESS_COARSE_LOCATION
	 * 491 33.65% READ_CALL_LOG 467 32.01% READ_CONTACTS 467 32.01% WAKE_LOCK
	 * 441 30.27% ACCESS_FINE_LOCATION 430 29.51% CALL_PHONE 426 29.24%
	 * CHANGE_WIFI_STATE 392 26.91% WRITE_CALL_LOG 376 25.81% WRITE_CONTACTS 376
	 * 25.81% INSTALL_PACKAGES 358 24.57% WRITE_APN_SETTINGS 357 24.50%
	 * RESTART_PACKAGES 341 23.40%
	 */

	public static String[] top30Permissions = { READ_PHONE_STATE, // 1
			INTERNET, // 2
	 ...
	 * RECEIVE_SMS, READ_SMS, ACCESS_COARSE_LOCATION, READ_CONTACTS,
	 * ACCESS_FINE_LOCATION, WRITE_SMS, CALL_PHONE, WAKE_LOCK,
	 * CHANGE_CONFIGURATION, READ_LOGS, WRITE_CONTACTS, RECEIVE_WAP_PUSH,
	 * WRITE_APN_SETTINGS, SYSTEM_ALERT_WINDOW, CAMERA, GET_TASKS,
	 * WRITE_SETTINGS, MOUNT_UNMOUNT_FILESYSTEMS, RECEIVE_MMS, BLUETOOTH,
	 * MODIFY_AUDIO_SETTINGS, CHANGE_WIFI_STATE, WRITE_CALENDAR,
	 * CHANGE_NETWORK_STATE, READ_CALENDAR, MODIFY_PHONE_STATE };
	 */
	...
	public static int getScoreForAllPerms(float top10Percentage,
			float top20Percentage, float top30Percentage) {
		int overallScore = 0;
		overallScore = getScoreForPerm(top30Percentage, TOP30_PERM_TYPE);
		System.out.println("Overall 1: " + overallScore);
		overallScore = overallScore
				+ getScoreForPerm(top20Percentage, TOP20_PERM_TYPE);
		System.out.println("Overall 2: " + overallScore);
		overallScore = overallScore
				+ getScoreForPerm(top10Percentage, TOP10_PERM_TYPE);
		System.out.println("Overall 3: " + overallScore);
		return overallScore;
	}

	public static int getScoreForAllClassifications(float critPercent,
			float highPercent, float medPercent, float lowPercent) {
		int score = 0;
	...
	public static int getDeviceFactorScore(int numberThirdPartyApps,
			String unlockSettingType, int unlockSettingLength,
		...
		score = score
				- getScoreForUnlockSettings(unlockSettingType,
						unlockSettingLength);
	...
	public static int getScoreForUnlockSettings(LockPatternUtils lockUtils) {
		...
		if (lockUtils.isLockPasswordEnabled()) {
			result = AnalyticsUtils.getScoreForUnlockSettings(
					AnalyticsUtils.UNLOCK_SETTING_PASS,
					lockUtils.getRequestedMinimumPasswordLength());
		} else if (lockUtils.isLockPatternEnabled()) {
			result = AnalyticsUtils.getScoreForUnlockSettings(
					AnalyticsUtils.UNLOCK_SETTING_PATTERN, 0);
		} else {
			result = AnalyticsUtils.getScoreForUnlockSettings(
					AnalyticsUtils.UNLOCK_SETTING_NONE, 0);
		}

		return result;
	}

	public static int getScoreForUnlockSettings(String unlockSettingType,
			int length) {
	...
	public static int getAppAgeScore(float percentOneYearOld,
			float percentSixMonthsOld) {
	...
	public static int getAppClassification(int top10, int top20, int top30) {
		...
		if (top10 >= CRIT_TOP10_AMOUNT || top20 >= CRIT_TOP20_AMOUNT
				|| top30 >= CRIT_TOP30_AMOUNT)
			classification = CRITICAL;
		else if (top10 >= HIGH_TOP10_AMOUNT || top20 >= HIGH_TOP20_AMOUNT
				|| top30 >= HIGH_TOP30_AMOUNT)
			classification = HIGH;
		else if (top10 >= MED_TOP10_AMOUNT || top20 >= MED_TOP20_AMOUNT
				|| top30 >= MED_TOP30_AMOUNT)
	...
	public static boolean isAppThirdParty(PackageManager packageManager,
			String packageName) {
	...
	public static boolean isThisPackage(String packageName) {
		return packageName
				.equals("com.device.security.analytics.androidsecurityanalyticspro")
				|| packageName
						.equals("com.device.security.analytics.androidsecurityanalytics");
	}

	public static boolean isAppInTrustedList(ArrayList<TrustedApp> trustedApps,
			String packageName) {
		for (Iterator iterator = trustedApps.iterator(); iterator.hasNext();) {
			TrustedApp trustedApp = (TrustedApp) iterator.next();
			if (trustedApp.getPackageName().equals(packageName))
				return true;
		}
		return false;
	}
